...
class @SkeletonVisualization
  ...
  _initOptions: ->
    @_scale = 0.1
    settings = Settings.viewer.threeD.skeleton
  ...
  _updateSkeleton: (skeleton) ->
    for joint, position of skeleton
      object = if joint == 'head'
        @_head
      else
        @_boxes[joint]
      @_updateObject object, position if object?
    for line in @_lines
      line.visible = true
      line.geometry.verticesNeedUpdate = true

  _updateObject: (object, position) ->
    object.position.x = position.x * @_scale
    object.position.y = position.y * @_scale
    object.position.z = position.z * -@_scale
